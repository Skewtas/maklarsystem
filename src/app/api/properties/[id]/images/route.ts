...
async function uploadImageToStorage(
  ...
  try {
    ...
    // Upload original image (private bucket). RLS owner will be auth.uid().
...
export async function POST(
  ...
  try {
    ...
    if (!file) {
      return NextResponse.json({ success: false, message: 'Ingen fil skickades' }, { status: 400 })
    }

    // Upload to storage
    const { path, error } = await uploadImageToStorage(supabase, propertyId, file, user.id)
    if (error) {
      return NextResponse.json({ success: false, message: error }, { status: 400 })
    }

    // Create DB row (storage path kept in `path`, here we store the full url for now)
    const { error: insertErr } = await supabase
      .from('property_images')
      .insert({ objekt_id: propertyId, user_id: user.id, path, thumbnail_path: path, caption: null })

    if (insertErr) {
      return NextResponse.json({ success: false, message: insertErr.message }, { status: 400 })
    }

    return NextResponse.json({ success: true, message: 'Bild uppladdad', data: { path } })
