-- ============================================
-- Migration: Performance Optimization Indexes (Fixed)
-- Date: 2025-08-16
-- Description: Additional indexes for query performance with IMMUTABLE functions
-- ============================================
...
-- Create IMMUTABLE function for concatenating names
CREATE OR REPLACE FUNCTION concat_name_immutable(fornamn text, efternamn text) 
RETURNS text AS $$
BEGIN
    RETURN COALESCE(fornamn, '') || ' ' || COALESCE(efternamn, '');
END;
...
CREATE INDEX IF NOT EXISTS idx_kontakter_namn_trgm ON kontakter USING gin (concat_name_immutable(fornamn, efternamn) gin_trgm_ops);
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifikationer') THEN
...
-- Analyze these only if they exist
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'notifikationer') THEN
