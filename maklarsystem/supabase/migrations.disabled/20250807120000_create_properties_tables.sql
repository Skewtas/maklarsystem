...
-- Avoid requiring PostGIS locally; use plain POINT and remove ST_* dependencies
...
CREATE TABLE properties (
  ...
  -- Constraints (skip ST_X/ST_Y locally)
...
-- Text search indexes
CREATE INDEX idx_properties_title_gin ON properties USING gin(to_tsvector('simple', title));
CREATE INDEX idx_properties_description_gin ON properties USING gin(to_tsvector('simple', full_description));
CREATE INDEX idx_properties_address_gin ON properties USING gin(to_tsvector('simple', street || ' ' || city));

-- Geospatial index for location-based searches
-- Skip PostGIS gist index locally
...
BEGIN
  ...
  SELECT 
    ...
    ts_rank_cd(
      ...
      setweight(to_tsvector('simple', p.full_description), 'C') ||
      setweight(to_tsvector('simple', p.street || ' ' || p.city), 'D'),
